...
import type { AIProvider, BackgroundAnalyzeVideoPayload, ExtensionConfig, VideoAnalysisResult } from "@/shared/types";
  ...
  return {
    ...
    async analyzeVideo(payload, config) {
      ...
      try {
        ...
        const { result, rawResponse } = await callAiProvider(client, ensureOriginAccess, config, filtered.text, payload.topComment, signal);
        return {
          ...result,
          danmakuCount: filtered.count,
          rawResponse
